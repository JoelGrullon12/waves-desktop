...
export function PlaylistView() {
  ...
  const { data, isLoading, isError, error } = useQuery({
    ...
    queryFn: async (): Promise<Track[]> => {
      ...
      const result = await tidalCatalogService.getPlaylistTracks(playlistId);
  ...
  const source: PlaybackSource = { sourceType: "playlist", sourceId: playlistId ?? "playlist" };
  const firstTrackArtist = data?.[0]?.artists[0]?.name;
    ...
    <div className="p-6">
      <header className="mb-6 flex items-end gap-4">
        ...
        <div className="min-w-0">
          ...
          <h1 className="truncate text-2xl font-bold">Playlist</h1>
          <p className="mt-0.5 truncate text-sm text-muted-foreground">
            {firstTrackArtist ? `Featuring ${firstTrackArtist}` : ""}
